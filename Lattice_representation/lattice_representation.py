...
# get data points within certain cutoff from initial LAMMPS data file by python embedded ovito
def get_data_points (file):
    pipeline = import_file((os.path.join(__location__,file)), atom_style = 'atomic')
    pipeline.modifiers.append(ExpressionSelectionModifier(expression = '(Position.X-39.4368)^2+(Position.Y-16.6971)^2>91.72'))
    pipeline.modifiers.append(DeleteSelectedModifier())
    pipeline.modifiers.append(AffineTransformationModifier(transformation = [[1.0, 0.0, 0.0, -30.2], [0.0, 1.0, 0.0, -9.1], [0.0, 0.0, 1.0, 0.0]], 
